...
<div class="right_text">
    <? foreach ( $this->fields as $key => $row ) : ?>
        <? if ( isset ( $row[ 'selected' ] ) ) : ?>
            <a class="order" order_type="<?= $row[ 'order' ] ?>" field="<?= $key ?>"><?= $row[ 'text' ] ?></a>
        <? else : ?>
            <a class="order" field="<?= $key ?>"><?= $row[ 'text' ] ?></a>
        <? endif; ?>
    <? endforeach; ?>
        ...
        <div id="c_<?= $row[ 'id' ] ?>">
            ...
            <div class="comment">
                <div class="head" <?= (!empty ( $row[ 'background-header-color' ] ) ? 'style="background-color : #' . $row[ 'background-header-color' ] . ';" title="' . $row[ 'group_name' ] . '"' : '' ) ?>>
                    ...
                    <span class="date_info small"><?= Output::date_format ( $row[ 'comment_date_ins' ], true ) ?> <?= ( Auth::rule ( DB\Rules::COMMENTS_DELETE ) ? ( '( ' . $row[ 'ip' ] . ' )' ) : '' ) ?></span>
                </div>
