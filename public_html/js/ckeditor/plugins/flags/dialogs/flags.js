...
CKEDITOR.dialog.add( 'flags', function( editor )
{
	var config = editor.config,
		images = config.flags_images,
		columns = config.flags_columns || 8,
		i;
	...
	var labelId = CKEDITOR.tools.getNextId() + '_flags_emtions_label';
	var html =
	[
		'<div>' +
	...
	for ( i = 0 ; i < size ; i++ )
	{
		...
		var flagsLabelId = 'cke_smile_label_' + i + '_' + CKEDITOR.tools.getNextNumber();
		html.push(
			'<td class="cke_dark_background cke_centered" style="vertical-align: middle;">' +
				'<a style="width:50px;" href="javascript:void(0)" role="option"',
					...
					'<img style="margin:3px;" class="cke_hand" title="', config.flags_descriptions[i], '"' +
						' cke_src="', CKEDITOR.tools.htmlEncode( config.flags_path + images[ i ] ), '" alt="', config.flags_descriptions[i], '"',
						' src="', CKEDITOR.tools.htmlEncode( config.flags_path + images[ i ] ), '"',
						// IE BUG: Below is a workaround to an IE image loading bug to ensure the image sizes are correct.
						( CKEDITOR.env.ie ? ' onload="this.setAttribute(\'width\', 2); this.removeAttribute(\'width\');" ' : '' ),
					'>' +
					'<span style="display: block;font-size:10px;text-align: center;color:#444;" id="' + flagsLabelId + '">' +config.flags_descriptions[ i ]  + '</span>' +
				'</a>',
 			'</td>' );
