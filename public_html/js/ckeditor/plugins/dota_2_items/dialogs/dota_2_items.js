...
*/
CKEDITOR.dialog.add( 'dota_2_items', function( editor )
{
	var config = editor.config,
		images = config.dota_2_items_images,
		columns = config.dota_2_items_columns || 8,
		i;
	...
	var labelId = CKEDITOR.tools.getNextId() + '_dota_2_items_emtions_label';
	var html =
		[
			'<div>' +
	...
	for ( i = 0 ; i < size ; i++ )
	{
		...
		var dota_2_itemsLabelId = 'cke_smile_label_' + i + '_' + CKEDITOR.tools.getNextNumber();
		html.push(
			...
			' aria-posinset="' + ( i +1 ) + '"',
			' aria-setsize="' + size + '"',
			' aria-labelledby="' + dota_2_itemsLabelId + '"',
			' class="cke_smile cke_hand" tabindex="-1" onkeydown="CKEDITOR.tools.callFunction( ', onKeydown, ', event, this );">',
			'<img style="margin:3px;" class="cke_hand" title="', config.dota_2_items_descriptions[i], '"' +
			' cke_src="', CKEDITOR.tools.htmlEncode(  images[ i ] ), '" alt="', config.dota_2_items_descriptions[i], '"',
			' src="', CKEDITOR.tools.htmlEncode( images[ i ] ), '"',
			// IE BUG: Below is a workaround to an IE image loading bug to ensure the image sizes are correct.
			( CKEDITOR.env.ie ? ' onload="this.setAttribute(\'width\', 2); this.removeAttribute(\'width\');" ' : '' ),
			'>' +
			'<span style="display: block;font-size:10px;text-align: center;color:#444;" id="' + dota_2_itemsLabelId + '">' +config.dota_2_items_descriptions[ i ]  + '</span>' +
			'</a>',
			'</td>' );
	...
	var dota_2_itemsSelector =
		{
			type : 'html',
			id : 'dota_2_itemsSelector',
			html : html.join( '' ),
